...
function initialize(sails){
  ...
  return (next) => {
    const connection = sails.config.connections[sails.config.models.connection],
      ...
      modelList = [];
    let sequelize;
    const ns = cls.createNamespace(connection.cls);
    ...
    return sails.modules.loadModels(function(err, models) {
      ...
      if(migrate === "alter"){
        return sequelize.sync({ alter: true, }).asCallback(next);
      }else if (migrate === "drop"){
        return sequelize.sync({ force: true, }).asCallback(next);
      }else{
        return next();
      }
    });
